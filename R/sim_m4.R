...
#'
#' @examples
sim_m4=function(pars=list(I=2,J=3,K=5,w_a=w_a,sigma_ag=1,u_mean=0,sigma_mean=1,u_env=0,sigma_env=1, u_gen=0,sigma_env=1,u_epsilon=0,sigma_epsilon=1, x_env=x_env,sigma_w=1,u_w=0,sigma_ge=1,u_ge=0)){
  # setting the dimensions
  I=pars$I
  J=pars$J
  env_labels=sprintf("environment_%s",seq(1:J))
  gen_label=sprintf("genotype_%s",seq(1:I))
  df_m4=expand.grid(environment_label=env_labels,genotype_label=gen_label)
  ##creating incidence matrix

  library(modelr)
  inc=model_matrix(~.-1, data=df_m4,  contrasts.arg =  lapply(df_m4[, sapply(df_m4, is.factor), drop = FALSE],
  ...
  for (i in 1:length(df_train_test)){
    ...
    df_m4$gen_effect=rep(g_i,each=J)
    df_m4$env_effect=rep(e_j,I)
    df_m4$epsilon=epsilon_ij
    df_m4$mu=rep(rnorm(1),I*J)
    df_m4$w_k=w_k
    df_m4$ge=ge
    df_m4$y=(df_m4$gen_effect+df_m4$env_effect+df_m4$epsilon+df_m4$mu+df_m4$w_k+df_m4$ge)
    df_train_test[[i]]<-df_m4
  }
